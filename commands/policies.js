...
export const policies = (androidApi, config) => {
  const policiesCommand = new Command('policies').description(
  ...
  policiesCommand
    .command('patch')
    .description(
      'Add/update a policy. Learn how to make policy JSON: https://developers.google.com/android/management/reference/rest/v1/enterprises.policies'
    )
    .requiredOption(
      '-f, --file <file-path>',
      'Required. Specify path to policy JSON file.'
    )
    .requiredOption(
      '-i, --id <policy-id>',
      'Required. Specify the ID of the policy, to reference in enrollment token or when adding to a device.'
    )
    .option(
      '-e, --enterprise-name <enterprise-name>',
      "Specify the name of Android Enterprise to add or update policy for. Skip if 'defaultEnterprise' is set in the config."
    )
    .action((options) => {
      let policyName = null;
      ...
      if (fs.existsSync(policyFilePath)) {
        ...
        androidApi.enterprises.policies
          .patch({
            name: policyName,
          ...
          .then((patchPoliciesResponse) => {
            ...
            console.log('');
            console.log(
              chalk.blue(`'${options.id}' policy successfully added/updated.`)
            );
            console.log('');
  ...
  policiesCommand
    ...
    .option(
      '-e, --enterprise-name <enterprise-name>',
      "Specify the name of Android Enterprise to list policies from it. Skip if 'defaultEnterprise' is set in the config."
    )
    .action((options) => {
      let enterpriseName;
      ...
      androidApi.enterprises.policies
        ...
        .then((listPoliciesResponse) => {
          ...
          if (!policies) {
            ...
            console.log(
              chalk.yellow(
                "No policies available. Use 'policies patch' command to add a policy."
              )
            );
            return;
          }
          ...
          policies.forEach((policy) => {
            ...
            table.push([policyId, policy.version]);
          });
        ...
        .catch((error) => {
          console.log('');
          console.error(
            chalk.red("Couldn't get policies:"),
            error.code + ':',
            error.message
          );
        });
  ...
  policiesCommand
    .command('get')
    .description(
      "Get policy details. Run 'policies list' command to get ID of the policy."
    )
    .requiredOption(
      '-i, --id <policy-id>',
      'Required. Specify the ID of the policy to get details.'
    )
    .option(
      '-e, --enterprise-name <enterprise-name>',
      "Specify the name of Android Enterprise to get policy from it. Skip if 'defaultEnterprise' is set in the config."
    )
    .action((options) => {
      let enterpriseName;
  ...
  policiesCommand
    .command('delete')
    .description(
      "Delete policy from your Android Enterprise. Run 'policies list' command to get ID of the policy."
    )
    .requiredOption(
      '-i, --id <policy-id>',
      'Required. Specify the ID of the policy to delete.'
    )
    .option(
      '-e, --enterprise-name <enterprise-name>',
      "Specify the name of Android Enterprise to delete policy from it. Skip if 'defaultEnterprise' is set in the config."
    )
    .action((options) => {
      let enterpriseName;
      ...
      androidApi.enterprises.policies
        ...
        .then(() => {
          ...
          console.log('');
          console.log(
            chalk.green(`${options.id} policy successfully deleted.`)
          );
          console.log('');
        })
        .catch((error) => {
          console.log('');
          console.error(
            chalk.red("Couldn't delete policy:"),
            error.code + ':',
            error.message
          );
        });
